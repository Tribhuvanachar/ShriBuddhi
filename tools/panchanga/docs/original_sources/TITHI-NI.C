...
{
int cnt=0;
float m=257504;double ay=0.745;
...
double ssrf,ssrii,ssrc,chk,rs,cm,scm1,scm,a1,b,cuk,cu1_frac,mscx,x,x_frac,no_tithi,tithi;
...
cm1=cm=cm+1.224157;//-0.000833;deemed correction for tithinirnaya
if(cm>12)
cm=cm-12;
printf("\n cm=%lf",cm);
...
cu1=cu1+6.109093;//-0.006935;deemed correction
//printf("\ncu1=%lf",cu1);
if(cu1>12)
cu1=cu1-12;
cu=cu1+0.003712;
...
// printf("\nssr=%lf",ssr);
 ...
 chk=1110+((ssr-10)*108.7);
 else if(ssr<=30)
 chk=2197+((ssr-20)*106.5);
  else if(ssr<=40)
 chk=3262+((ssr-30)*99.8);
  else if(ssr<=50)
 chk=4250+((ssr-40)*87.0);
  else if(ssr<=60)
 chk=5130+((ssr-50)*73.8);
  else if(ssr<=70)
 chk=5868+((ssr-60)*59.5);
  else if(ssr<=80)
 chk=6463+((ssr-70)*36.2);
  else if(ssr<=90)
 chk=6825+((ssr-80)*13.0);//in vikale
