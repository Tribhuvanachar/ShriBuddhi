...
reproducing the acquisition documented in `sources/README.md` and
`PENDING.md` (11 Sep 2026 entries) as a re-runnable tool instead of a
one-off manual pass.
...
- writes to `sources/<institution>/panchanga/<derived-date-range>/`
...
(see `sources/README.md` for the details on each):
