...
Three sources, and it says which fact came from which:
  ...
  admin/config/ocr_sarvam_missing.json  the pages STILL missing today, which
                                   is what a rerun would cost. Regenerated by
                                   tools/rebuild_sarvam_missing.py; not the
                                   same as the historical refusal tally.
...
def build(root="."):
    ...
    # pages_refused is a HISTORICAL tally: every page an engine ever
    # refused, counted from the per-file error blocks. It never goes down,
    # because a re-dispatch writes a NEW staged file and the original keeps
    # its failed entries forever. So it is the wrong number to price a rerun
    # from -- on 22 Sep it said 5,094 pages and INR 2,241 when 1,059 of those
    # had already been re-fetched and paid for, and only 4,283 were actually
    # outstanding. The outstanding count lives in
    # admin/config/ocr_sarvam_missing.json, which tools/rebuild_sarvam_missing.py
    # regenerates from the staged files, resolving each page across every file
    # of its work. MISSING was declared here since this file was written and
    # never read; now it is.
    outstanding = grand["pages_refused"]
    try:
        with open(MISSING, encoding="utf-8") as fh:
            outstanding = json.load(fh).get("total_pages_missing", outstanding)
    except (OSError, ValueError):
        pass
    grand["pages_outstanding"] = outstanding
    grand["recoverable_by_rerun_inr"] = round((sarvam_rate or 0) * outstanding, 2)

    # Per work, the same distinction the grand total makes: pages_refused is
    # what was ever refused, pages_outstanding is what still has no OCR. The
    # admin page's "pages still to run" badge keyed off the former, so works
    # fully re-fetched on 22 Sep still advertised themselves as unfinished --
    # prashna showed "287 failed, pages still to run" after all 287 came back.
    outstanding_by_work = {}
    try:
        with open(MISSING, encoding="utf-8") as fh:
            for wk, files in (json.load(fh).get("works") or {}).items():
                outstanding_by_work[wk] = sum(f.get("pages") or 0 for f in files)
    except (OSError, ValueError):
        pass
    for w in works.values():
        w["pages_outstanding"] = outstanding_by_work.get(w["work"], 0)
    ...
    for w in works.values():
        ...
        for k in ("inr", "pages_billed", "pages_failed", "pages_refused",
                  "pages_outstanding", "pages_server_error", "staged_files"):
            g[k] += w.get(k) or 0
...
def main(argv=None):
    ...
    print("pages ever refused (historical) %d  (never decreases -- a rerun "
          "writes a new file)" % g["pages_refused"])
    print("pages still outstanding         %d  -> rerun costs INR %.2f"
          % (g["pages_outstanding"], g["recoverable_by_rerun_inr"]))
