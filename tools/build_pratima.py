#!/usr/bin/env python3
"""Build guru-parampara/data/pratima.json -- the deity/pratima layer
...
(currently: sources/vyasaraja_anjaneya_app/), one source per run, never
guessed. Re-run this after adding a new pratima source to sources/.
...
HANUMAN_SRC = os.path.join(ROOT, "sources/vyasaraja_anjaneya_app/vyasaraja_hanuman_installations.json")
UTTARADI_DB = os.path.join(ROOT, "sources/uttaradi_matha/panchanga/2024-2027/um_app_seed.db")
OUT = os.path.join(ROOT, "guru-parampara/data/pratima.json")
...
def build_vyasaraja_hanuman_entries():
    ...
    for row in rows:
        ...
        entries.append({
            ...
            "image": None,
            "source": {
                "artifact": "sources/vyasaraja_anjaneya_app/vyasaraja_hanuman_installations.json",
                "manifest": "sources/vyasaraja_anjaneya_app/source-manifest.json",
            },
        })
    return entries
...
def build_uttaradi_deity_entries():
    ...
    for row in cur.fetchall():
        ...
        entries.append({
            ...
            "image": r["image_path"],
            "source": {
                "artifact": "sources/uttaradi_matha/panchanga/2024-2027/um_app_seed.db",
                "manifest": "sources/uttaradi_matha/panchanga/2024-2027/source-manifest.json",
...
def main():
    ...
    out = {
        "_readme": [
            ...
            "acquired source data in sources/, one source at a time, by",
